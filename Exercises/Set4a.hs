...
incrementKey _ [] = []
incrementKey targetK ((k, v) : xs)
  | k == targetK = (k, v + 1) : incrementKey targetK xs
  | otherwise    = (k, v)     : incrementKey targetK xs
...
freqs xs = foldr update Map.empty xs
  where
    -- update takes the current element and the map built so far
    update x currentMap = Map.alter increment x currentMap
    
    -- increment handles the logic for a single key
...
transfer from to amount bank = 
  case (Map.lookup from bank, Map.lookup to bank) of
    (Just balanceFrom, Just balanceTo) 
      | amount >= 0 && balanceFrom >= amount ->
          let bankWithdrawn = Map.adjust (\x -> x - amount) from bank
          in Map.adjust (\x -> x + amount) to bankWithdrawn
    _ -> bank
...
maxIndex arr = 
  let pairs = assocs arr
      -- Use a helper to find the pair with the largest second element
      bestPair = foldr1 (\(idx, val) (bestIdx, bestVal) -> 
