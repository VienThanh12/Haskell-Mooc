...
countHelper x acc = maybe (acc + 1) (\_ -> acc) x
------------------------------------------------------------------------------
...
maxHelper x currentMax = max x currentMax
------------------------------------------------------------------------------
...
slHelper x (currentSum, currentLength) = (x + currentSum, 1 + currentLength)
------------------------------------------------------------------------------
...
concatHelper :: [a] -> [a] -> [a]
concatHelper currentList joinedRest = currentList ++ joinedRest
...
largestHelper x acc@(y:_)
  | x > y     = [x]      -- Found a new maximum, discard old ones
  | x == y    = x : acc  -- Found another occurrence of the maximum
  | otherwise = acc      -- x is smaller, ignore it
